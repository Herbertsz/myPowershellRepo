...
+---------------------------------------------------------------------------------------+
|	Fixes: Herbert Szumovski (HS)														|
|	2019-12-14, Version  9.5.3.1:                                                       |
|			Updated to support V10                                                      |
|	2020-10-04, Version 11.0.0.0:                                                       |
|			Updated to support V11 Beta                                                 |
|	2021-01-27, Version 11.0.0.1:                                                       |
|			Support of V11 GA, removed powershell snapin, replaced deprecated calls     |
|	2022-03-12, Version 11.0.1.1:                                                       |
...
#region old VersionInfo (for new versions plz see scriptheader)
$MVRversion = "11.0.1.3"
# Version 9.5.3 - SM
...
$allJobsBc = @($allJobs | ?{$_.JobType -eq "BackupSync" -or $_.JobType -eq "SimpleBackupCopyPolicy"})
# Get all Tape Jobs
$allJobsTp = @()
...
$totalXferBk = 0
$totalReadBk = 0
$sessListBk | %{$totalXferBk += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB,0))}
$sessListBk | %{$totalReadBk += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB,0))}
...
$totalXferRp = 0
$totalReadRp = 0
$sessListRp | %{$totalXferRp += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB,0))}
$sessListRp | %{$totalReadRp += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB,0))}
...
$totalXferBc = 0
$totalReadBc = 0
$sessListBc | %{$totalXferBc += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB,0))}
$sessListBc | %{$totalReadBc += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB,0))}
...
$totalXferTp = 0
$totalReadTp = 0
$sessListTp | %{$totalXferTp += $([Math]::Round([Decimal]$_.Progress.TransferedSize/1GB,0))}
$sessListTp | %{$totalReadTp += $([Math]::Round([Decimal]$_.Progress.ReadSize/1GB,0))}
...
Function Get-VBRRepoInfo {
  ...
  Begin {
    $outputAry = @()
    Function Build-Object {param($name, $repohost, $path, $free, $total, $maxtasks, $rtype, $rBackupsize)
      $repoObj = New-Object -TypeName PSObject -Property @{
        Target = $name
        RepoHost = $repohost
        Storepath = $path
        StorageFree = [Math]::Round([Decimal]$free/1GB,0)
        StorageTotal = [Math]::Round([Decimal]$total/1GB,0)
        FreePercentage = [Math]::Round(($free/$total)*100)
        StorageBackup = [Math]::Round([Decimal]$rBackupsize/1GB,0)
        StorageOther = [Math]::Round([Decimal]($total-$rBackupsize-$free)/1GB-0.5,0)
        MaxTasks = $maxtasks
        rType = $rtype
      }
      Return $repoObj
  ...
  Process {
    Foreach ($r in $Repository) {
      ...
      $outputObj = Build-Object $r.Name $($r.GetHost()).Name.ToLower() $r.Path $r.GetContainer().CachedFreeSpace.InBytes $r.GetContainer().CachedTotalSpace.InBytes $maxTaskCount $rType $rBackupSize
    }
    $outputAry += $outputObj
  }
  End {
...
Function Get-VBRSORepoInfo {
  ...
  Begin {
    $outputAry = @()
    Function Build-Object {param($name, $rname, $repohost, $path, $free, $total, $maxtasks, $rtype, $rBackupsize)
      $repoObj = New-Object -TypeName PSObject -Property @{
        SoTarget = $name
        Target = $rname
        RepoHost = $repohost
    ...
    Foreach ($obj in $InputObj) {
      If (($dsAry -contains $obj.ViReplicaTargetOptions.DatastoreName) -eq $false) {
        ...
        $objoutput = New-Object -TypeName PSObject -Property @{
          ...
          StorageFree = [Math]::Round([Decimal]$dtstr.FreeSpace/1GB,0)
          StorageTotal = [Math]::Round([Decimal]$dtstr.Capacity/1GB,0)
...
function Get-BackupSize {
  ...
  Foreach ($backup in $backups) {
    ...
    Foreach ($file in $Files) {
      $backupSize += [math]::Round([long]$file.Stats.BackupSize/1GB,0)
      $dataSize += [math]::Round([long]$file.Stats.DataSize/1GB,0)
    }         
    ...
    <body>
        <center>
            <table>
                ...
                <tr>
                    <td style="width: 50%;height: 12px;border: none;background-color: ZZhdbgZZ;color: White;font-size: 12px;vertical-align: bottom;text-align: left;padding: 0px 0px 0px 5px;"></td>
                    <td style="width: 50%;height: 12px;border: none;background-color: ZZhdbgZZ;color: White;font-size: 12px;vertical-align: bottom;text-align: right;padding: 0px 5px 0px 0px;">$($objectVersion.productHotfix)</td>
                </tr>
                <tr>
                    <td style="width: 50%;height: 12px;border: none;background-color: ZZhdbgZZ;color: White;font-size: 12px;vertical-align: bottom;text-align: left;padding: 0px 0px 2px 15px;">$rptMode</td>
...
<table>
                <tr>
                    <td style="height: 35px;background-color: #cccfff;color: #626365;font-size: 16px;padding: 5px 0 0 15px;border-top: 5px solid white;border-bottom: none;">
"@
...
<table>
                 <tr>
                    <td style="height: 35px;background-color: #FFC300;color: #ffffff;font-size: 16px;padding: 5px 0 0 15px;border-top: 5px solid white;border-bottom: none;">
"@

$subHead01err = @"
<table>
                <tr>
                    <td style="height: 35px;background-color: #900C3F;color: #ffffff;font-size: 16px;padding: 5px 0 0 15px;border-top: 5px solid white;border-bottom: none;">
"@
...
$HTMLbreak = @"
        <table>
                <tr>
                    <td style="height: 12px;background-color: #626365;padding: 5px 0 0 15px;border-top: 5px solid white;border-bottom: none;"></td>
				</tr>
        </table>
        </BR></BR></BR></BR>
        <table>
...
<table>
                <tr>
                    <td style="height: 15px;background-color: #ffffff;border: none;color: #626365;font-size: 10px;text-align:center;"></td>
                </tr>
            </table>
        </center>
    </body>
      ...
      $bodyJobsBk += $bkJob | Select @{Name="Job Name"; Expression = {$_.Name}},
        ...
        @{Name="Status"; Expression = {
          If ($bkJob.IsRunning) {
            ...
            $csessSpeed = [Math]::Round($currentSess.Progress.AvgSpeed/1MB,0)
  ...
  If ($sessListBk.count -gt 0) {
    If ($showDetailedBk) {
      $arrAllSessBk = $sessListBk | Sort Creationtime | Select @{Name="Job Name"; Expression = {$_.Name}},
        ...
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Progress.AvgSpeed/1MB,0)}},
        @{Name="Total (GB)"; Expression = {[Math]::Round($_.Progress.ProcessedSize/1GB,0)}},
        @{Name="Processed (GB)"; Expression = {[Math]::Round($_.Progress.ProcessedUsedSize/1GB,0)}},
        @{Name="Data Read (GB)"; Expression = {[Math]::Round($_.Progress.ReadSize/1GB,0)}},
        @{Name="Transferred (GB)"; Expression = {[Math]::Round($_.Progress.TransferedSize/1GB,0)}},
...
If ($showRunningBk) {
  If ($runningSessionsBk.count -gt 0) {
    $bodyRunningBk = $runningSessionsBk | Sort Creationtime | Select @{Name="Job Name"; Expression = {$_.Name}},
      ...
      @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},
      @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Progress.AvgSpeed/1MB,0)}},
      @{Name="Read (GB)"; Expression = {[Math]::Round([Decimal]$_.Progress.ReadSize/1GB,0)}},
      @{Name="Transferred (GB)"; Expression = {[Math]::Round([Decimal]$_.Progress.TransferedSize/1GB,0)}},
...
If ($showRunningTasksBk) {
  If ($runningTasksBk.count -gt 0) {
    $bodyTasksRunningBk = $runningTasksBk | Select @{Name="VM Name"; Expression = {$_.Name}},
        ...
        @{Name="Duration (HH:MM:SS)"; Expression = {Get-Duration -ts $_.Progress.Duration}},                    
        @{Name="Avg Speed (MB/s)"; Expression = {[Math]::Round($_.Progress.AvgSpeed/1MB,0)}},
        @{Name="Read (GB)"; Expression = {[Math]::Round($_.Progress.ReadSize/1GB,0)}},
        @{Name="Transferred (GB)"; Expression = {[Math]::Round($_.Progress.TransferedSize/1GB,0)}},
      ...
      $bodyJobsRp += $rpJob | Select @{Name="Job Name"; Expression = {$_.Name}},
        ...
        @{Name="Status"; Expression = {
          If ($rpJob.IsRunning) {
            ...
            $csessPercent = $currentSess.Progress.Percents
            $csessSpeed = [Math]::Round($currentSess.Info.Progress.AvgSpeed/1MB,0)
            $cStatus = "$($csessPercent)% completed at $($csessSpeed) MB/s"
            $cStatus
          } Else {
            "Stopped"
  ...
  If ($idleSessionsTp.count -gt 0) {
    ...
    If ($showDetailedTp) {
      $bodySessIdleTp = $idleSessionsTp | Sort Creationtime | Select @{Name="Job Name"; Expression = {$_.Name}},
        ...
        @{Name="Total (GB)"; Expression = {[Math]::Round($_.Info.Progress.ProcessedSize/1GB,0)}},
        @{Name="Data Read (GB)"; Expression = {[Math]::Round($_.Info.Progress.ReadSize/1GB,0)}},
        @{Name="Transferred (GB)"; Expression = {[Math]::Round($_.Info.Progress.TransferedSize/1GB,0)}} | ConvertTo-HTML -Fragment
...
If ($showTapes) {
  ...
  if($expTapes.Count -gt 0) {
    ...
    @{Name="Capacity (GB)"; Expression = {[Math]::Round([Decimal]$_.Capacity/1GB,0)}},
    @{Name="Free (GB)"; Expression = {[Math]::Round([Decimal]$_.Free/1GB,0)}},
...
If ($htmlOutput -match "#900C3F") {
  # If any errors paint report header red
  $htmlOutput = $htmlOutput.Replace("ZZhdbgZZ","#900C3F")
  $emailSubject = "[Failed] $emailSubject"
} ElseIf ($htmlOutput -match "#FFC300") {
  # If any warnings paint report header yellow
  $htmlOutput = $htmlOutput.Replace("ZZhdbgZZ","#FFC300")
